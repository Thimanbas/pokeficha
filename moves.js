...
const pokeTmTable = document.querySelector("#tm-move-table")
const pokeTutorTable = document.querySelector("#tutor-move-table")
...
const tmConfirmBtnFicha = document.querySelector('#tm-confirm-ficha');
const tutorConfirmBtnFicha = document.querySelector('#tutor-confirm-ficha');
const tmRefreshBtnFicha = document.querySelector('#tm-refresh-ficha');
const tutorRefreshBtnFicha = document.querySelector('#tutor-refresh-ficha');
...
try {
    ...
    tmConfirmBtnFicha.addEventListener('click', (e) => {
        ...
        for (i = 0; i < boxcheck.length; i++) {
            ...
            if (boxcheck[i].checked == false) {
                pokeTmTable.deleteRow(i - (boxcheck.length - pokeTmTable.rows.length))
                tmUnchecked.push({ name: `${boxcheck[i].value}` })
            }
    ...
    tmRefreshBtnFicha.addEventListener('click', (e) => {
        e.preventDefault();
        checkMoveFicha(tmUnchecked, 'tm')
        tmUnchecked = []
    ...
    tutorConfirmBtnFicha.addEventListener('click', (e) => {

        e.preventDefault();
        let boxtutorcheck = document.querySelectorAll('[id="tutor-checkbox"]')
        for (i = 0; i < boxtutorcheck.length; i++) {
            if (boxtutorcheck[i].checked == false) {
                pokeTutorTable.deleteRow(i - (boxtutorcheck.length - pokeTutorTable.rows.length))
                tutorUnchecked.push({ name: `${boxtutorcheck[i].value}` })
            }
    ...
    tutorRefreshBtnFicha.addEventListener('click', (e) => {
        e.preventDefault();
        checkMoveFicha(tutorUnchecked, 'tutor')
        tutorUnchecked = []
    })
} catch (e){
...
function moveHandlerFicha(data) {
    tutorMoves = []
    tmMoves = []
    arrTutorMoves = []
    ...
    checkMoveFicha(tutorMoves, "tutor")
    checkMoveFicha(tmMoves, "tm")
}



function checkMoveFicha(moves, type) {
    if (type == "tutor") {
        for (i = 0; i < moves.length; i++) {
            ...
            fetch(`https://pokeapi.co/api/v2/move/${moves[i].name}/`)
                ...
                .then(data => {
                    ...
                    createTutorMoveFicha(arrTutorMoves, moves.length)
                })
        }
    }
    if (type == "tm") {
        for (i = 0; i < moves.length; i++) {
            ...
            fetch(`https://pokeapi.co/api/v2/move/${moves[i].name}/`)
                ...
                .then(data => {
                    ...
                    createTmMoveFicha(arrTmMoves, moves.length)
                })
        }
    }
}
...
function createTutorMoveFicha(moveName, moveListLength) {
    moveTutorList.push(moveName)
...
function createTmMoveFicha(moveName, moveListLength) {
    moveTmList.push(moveName)
    if (moveTmList[0].length < moveListLength) return;
